...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        ...
        case HUNTER_PET:
        {
            ...
            SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(cinfo->mindmg)*8);
            //damage range is then petlevel / 2
            SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(cinfo->maxdmg)*8);
            //damage is increased afterwards as strength and pet scaling modify attack power
            SetModifierValue(UNIT_MOD_STAT_STAMINA, BASE_VALUE, float(m_owner->GetStat(STAT_STAMINA)) * 0.67f);  
            break;
        }
        default:
        {
            switch (GetEntry())
            {
                ...
                case 1964: //force of nature
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel * 7.5f - (petlevel / 2) + bonusDmg));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel * 7.5f + (petlevel / 2) + bonusDmg));
                    break;
                }
